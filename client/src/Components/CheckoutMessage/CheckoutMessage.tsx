...
import { checkoutMessageOpen, isCheckoutMessageOpen } from "../../redux/slices/checkout/checkoutMessageSlice"

const CheckoutMessage = () => {
  const dispatch = useAppDispatch()
  const  { isMessageOpen, message, isError }  =  useAppSelector(checkoutMessageOpen)
    ...
    <section className="relative">
      <article className={`absolute w-screen min-h-fit text-center z-50 shadow-lg text-lg text-gray-50 transform duration-500  ${isMessageOpen ? 'top-0' : '-top-28'} ${isError ? 'bg-red-500 ' : 'bg-green-500'} `}>
        <button
          ...
          onClick={() => dispatch(isCheckoutMessageOpen({ isOpen: false, message: '' }))}
        >
